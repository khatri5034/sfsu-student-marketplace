...
*/
function V(){let n=e(()=>{let e=m();if(typeof e.config==`object`&&`cookie_domain`in e.config){let t=e.config;if(typeof t.cookie_domain==`string`)return t.cookie_domain}}),r=e(()=>t()?!1:document.cookie.includes(B)),i=async()=>{await z(),y(`update`),window.location.reload()},a=async()=>{b(`update`),await d(B,n.value),window.location.reload()},o=async e=>{await z(),v(`update`,e),window.location.reload()};return r.value&&z(),{hasConsent:r,acceptAll:i,rejectAll:a,acceptCustom:o}}function H(){let{initMode:e}=m();e!==`manual`&&z()}
/**
